...
abalone %>% ggplot(aes(x = age)) + geom_bar()
```
It roughly follows a normal distribution skewed to the right.
...
```{r}
abalone_recipe = recipe(age ~ ., data = abalone_train) %>% step_rm(rings) %>% step_dummy(all_nominal_predictors()) %>% step_interact(terms = ~ starts_with("type"):shucked_weight + longest_shell:diameter+shucked_weight:shell_weight) %>% step_center(all_predictors()) %>% step_scale(all_predictors())
```
...
h1 = data.frame(rings = 0, type = "F", longest_shell = 0.50, diameter = 0.10, height = 0.30, whole_weight = 4, shucked_weight = 1, viscera_weight = 2, shell_weight = 1)

predict(lm_fit, new_data = h1)
```
### Question 7
...
result = predict(lm_fit,abalone_train) %>% bind_cols(abalone_train %>% select(age))
multi_metric(result, truth = age, estimate = .pred)
```
